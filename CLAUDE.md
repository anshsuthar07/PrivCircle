...
| --- | --- |
| [app/](app/) | App Router pages, API routes, client components, CSS modules |
...
- **Lifetime rooms** are durable in PostgreSQL (`lifetime_rooms` + `lifetime_documents`,
  one binary Yjs snapshot per room, cascade delete). Redis acts as an active cache;
  [lookupRoom()](lib/storage/rooms.ts#L68) rehydrates it from PostgreSQL on a miss.
...
| Route | Behavior |
| --- | --- |
| [POST /api/rooms](app/api/rooms/route.ts) | Create. Rate limit 10/h. Reserve-then-create with retry on generated paths. **No GET handler — rooms are never listable.** |
...
[serviceError()](lib/http.ts#L12) — Redis/PostgreSQL failures **fail closed** with `503`;
there is no in-memory authorization fallback.
...
[lib/realtime/presence.ts](lib/realtime/presence.ts) enforces the **two-participant limit** with
Lua over a Redis sorted set: 75 s leases keyed `participantId:socketId`, so a reconnect keeps
...
effect: Y.Doc + `HocuspocusProvider` + CodeMirror with `yCollab`. Language is a shared
...
- Secrets, tokens, passwords, request bodies, and Yjs updates are never logged — note the
  deliberately silent Redis error handler in [lib/redis.ts](lib/redis.ts#L19).
